...
class TestStellarRegistration:
    ...
    def test_successful_registration_in_firefox(self, driver2):
        ...
        driver2.find_element(*StellarLocators.NAME_FIELD_REG).send_keys(new_name)
        driver2.find_element(*StellarLocators.EMAIL_FIELD_REG).send_keys(new_email)
        driver2.find_element(*StellarLocators.PASSWORD_FIELD_REG).send_keys("123456789test")
        driver2.find_element(*StellarLocators.BUTTON_REGISTRATION).click()
        WebDriverWait(driver2, 10).until(
            expected_conditions.visibility_of_element_located(StellarLocators.PAGE_INPUT))
        driver2.get(f'{URL}login')
        driver2.find_element(*StellarLocators.EMAIL_FIELD_IN).send_keys(new_email)
        driver2.find_element(*StellarLocators.PASSWORD_FIELD_IN).send_keys("123456789test")
